...
**Status**: proposal submitted (3 Jun 2026); final presentation 24 Jun 2026. **Currently in Week 4-6 execution sprint** — Stage 1 models being built. See `docs/week2_onboarding.md` §4b for the model coordination table.
...
```
       ...
       MF · EASE · BPR · Sentence-BERT · hybrid · two-tower
            → returns top-200 candidates by predicted taste
                            │
                            ▼
...
**Stage 1** is interchangeable (we evaluate 4-8 models). **Stage 2** is the project's research contribution — studying the (αt, αp, αn) simplex via empirical α-sweep across two evaluation tracks (warm-item CF + cold-item content-aware).
...
│   ├── data_decisions.md             # 11 locked decisions with evidence
...
│   ├── models/
│   │   └── popularity.py             # Stage 1 baseline (more models incoming Week 2)
│   ├── eval/
│   │   ├── metrics.py                # Recall@K, NDCG@K, MRR
│   │   └── harness.py                # ⭐ Dual-track evaluation harness
│   ├── reranker/                     # Stage 2 reranker (Week 4 work)
│   ├── utils/
│   │   └── staples.py                # Universal kitchen staples + pantry_score
│   └── demo/                         # Demo widget (Week 5-6)
│
...
Only content-aware models (TF-IDF, Sentence-BERT, hybrid, two-tower) produce non-zero numbers on Track B. CF-only models score 0 by construction — this is **correct**, not a bug.
...
- **Cornac library** (MF, EASE, BPR): Salah et al., JMLR 2020
...
- **Two-tower neural**: Yi et al., RecSys 2019
